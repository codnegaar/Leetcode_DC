...
class Solution:
    def chalkReplacer(self, chalk: List[int], initialChalkPieces: int) -> int:
        ...
        remainingChalk = initialChalkPieces % totalChalk
        
        # Determine which student will run out of chalk
        for studentIndex, chalkRequired in enumerate(chalk):
            if remainingChalk < chalkRequired:
                return studentIndex
            remainingChalk -= chalkRequired
        
        # Fallback, though this line should never be reached
        return 0
