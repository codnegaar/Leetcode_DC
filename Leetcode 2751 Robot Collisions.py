...
If two robots collide, the robot with lower health is removed from the line, and the health of the other robot decreases by one. The surviving robot continues in the same 
direction it was going. If both robots have the same health, they are both removed from the line.
...
class Solution:
    def survivedRobotsHealths(self, positions: List[int], healths: List[int], directions: str) -> List[int]:
        ...
        # Create a list of robots with their attributes and original index
        for i in range(n):
            robots.append({
        ...
        for i in range(n):
            if robots[i]['direction'] == 'L':
                # Resolve collisions with right-moving robots in the stack
                ...
                if not stack or robots[stack[-1]]['direction'] == 'L':
                    ...
                    stack.pop()  # Same health, remove both robots
                elif stack and robots[stack[-1]]['health'] > robots[i]['health']:
                    robots[stack[-1]]['health'] -= 1  # Right-moving robot has greater health, decrease its health
            else:
                stack.append(i)  # Add right-moving robots to the stack
