...
class Solution:
    def findTheWinner(self, n: int, k: int) -> int:
        ...
        # Loop until only one friend remains in the circle
        while len(circle) > 1:
            # Determine the index of the next friend to be removed
            next_to_remove: int = (cur_ind + k - 1) % len(circle)
            # Remove the friend at the calculated index
            circle.pop(next_to_remove)
            # Update the current index to the next position after removal
