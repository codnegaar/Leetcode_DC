...
class Solution:
    def minimumCost(self, source: str, target: str, original: list[str], changed: list[str], cost: list[int]) -> int:
        # Initialize the cost array with 'inf' values to represent unreachable conversions
        arr = [[float('inf')] * 26 for _ in range(26)]
        
        # No cost for converting a letter to itself
        for i in range(26):
            arr[i][i] = 0
        
        # Fill in the initial direct conversion costs
        for i in range(len(original)):
            from_char = ord(original[i]) - ord('a')
            to_char = ord(changed[i]) - ord('a')
            arr[from_char][to_char] = min(arr[from_char][to_char], cost[i])
        
        # Use the Floyd-Warshall algorithm to find the shortest transformation path between all characters
        for k in range(26):
            for i in range(26):
                for j in range(26):
        ...
        # Calculate the total cost to transform the source into the target
        for i in range(len(source)):
            from_char = ord(source[i]) - ord('a')
            to_char = ord(target[i]) - ord('a')

            # If the characters are the same, no transformation is needed
            if from_char == to_char:
                continue

            # If there is no valid transformation, return -1
            if arr[from_char][to_char] == float('inf'):
                return -1
            
            # Add the transformation cost to the total
            total_cost += arr[from_char][to_char]
